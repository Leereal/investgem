        ...
        <div class="card-header card-header-rose card-header-icon">
          ...
          <h4 class="card-title">Bonus Total : <span class="text-success">${{$total}}</span></h4>
            <form action="/bonus-payall" method="POST">
                ...
                <button type="submit" class="btn btn-success btn-sm btn-round" onclick="confirm('Are you sure you want to pay this?')"><i class="material-icons">add_task</i>PAY</button>
            </form>
            <form action="/bonus-invest" method="POST">
                {{ csrf_field() }}
                <input type="hidden" name="user" value="{{$user}}">                        
                <button type="submit" class="btn btn-info btn-sm btn-round" onclick="confirm('Are you sure you want to invest this?')"><i class="material-icons">add_task</i>INVEST</button>
            </form>           
        </div>
        <div class="card-body">
